...
public class DatacenterBroker extends SimEntity {
	...
	protected void submitCloudlets() {
		Flower flower = new Flower(cloudletList, vmsCreatedList);
		List<ArrayList<Integer>> cAssign = flower.start();

		double answer = 0.0;
		int vmIndex = 0;
		int size = getVmsCreatedList().size();
		for (Cloudlet cloudlet : getCloudletList()) {
			Vm vm = getVmsCreatedList().get(cAssign.get(vmIndex).get(0));
			Log.printLine(CloudSim.clock() + ": " + getName() + ": Sending cloudlet "
					+ cloudlet.getCloudletId() + " to VM #" + vm.getId());
			bindCloudletToVm(cloudlet.getCloudletId(),vm.getId());

			answer += cloudlet.getCloudletLength() / vm.getMips();

			sendNow(getVmsToDatacentersMap().get(vm.getId()), CloudSimTags.CLOUDLET_SUBMIT, cloudlet);
			cloudletsSubmitted++;
			vmIndex++;
			if (vmIndex >= getVmsCreatedList().size()) vmIndex = 0;
		...
		System.out.println("MAKESPAN(SECONDS) - " + 3.5 * answer / size);
	}
	/**
